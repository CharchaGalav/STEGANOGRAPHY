...
def generate_embedded_imagename(vessel_img):
    #vessel_img :- D:/PycharmProjects/pproject/STEGANOGRAPHY/child.png
    #embedded_img :- D:/PycharmProjects/pproject/STEGANOGRAPHY/e_child.png
    if '/' in vessel_img:
        temp = vessel_img.split('/') # ['D:', 'PycharmProjects', 'pproject', 'STEGANOGRAPHY', 'child.png']
        temp[-1] = 'e_' + (temp[-1]) # ['D:', 'PycharmProjects', 'pproject', 'STEGANOGRAPHY', 'e_child.png']
        ename = '/'.join(temp) #D:/PycharmProjects/pproject/STEGANOGRAPHY/e_child.png
...
def embed(vessel_image , doc):
    #does vessel_image and doc exists?
    ...
    if header_length + doc_size > capacity:
    ...
    #open a file for reading in binary mode (to support reading all file types)
    ...
    while i < h and flag:#for each row
        j = 0
        while j < w and flag:# for each column of row i
            #fetch a pixel
            pixel = mem_image[i,j]
            blue = pixel[0]
            green = pixel[1]
            ...
            if cnt < header_length:
                # fetch a byte from header
                data = ord(header[cnt])
            else:
                #fetch a byte from file
                data = file_handle.read(1)
                if data:#test
                    #data fetched is in form :byte object
                    #it need conversion in int
            ...
            #update the mim_image_pixel[i,j]
